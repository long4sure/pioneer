...
function utilCalcAll() {
    ...
    utilLines.forEach(l => {
        ...
        tbl += `<tr><td><strong>${utilLineNames[l]}</strong></td><td>${fmtN(prod)}</td><td>${fmtN(tot)}</td><td>${u.toFixed(1)}%</td><td>${d.mAvail || 0}/${d.mUsed || 0}</td><td><span class="badge ${sc}">${st}</span></td></tr>`;
    });

    const ou  = tTime > 0 ? tProd / tTime * 100 : 0;
    const avg = utilArr.length > 0 ? utilArr.reduce((a, b) => a + b, 0) / utilArr.length : 0;

    setText('util-overall', ou.toFixed(1) + '%');
    ...
    setText('util-avgUtil', avg.toFixed(1) + '%');
