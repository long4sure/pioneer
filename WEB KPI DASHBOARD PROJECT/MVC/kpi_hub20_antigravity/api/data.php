...
function handleLoad(): never {
    ...
    $module  = $_GET['module'] ?? '';
    $yearRaw = $_GET['year']   ?? '0';
    // Special case: year=all returns all saved periods for this module
    if ($yearRaw === 'all') { loadAllPeriods($module); }
    $year  = (int)$yearRaw;
...
// =============================================================
// LOAD ALL PERIODS (for sysadmin Data Records)
// =============================================================
function loadAllPeriods(string $module): never {
    ...
    $stmt = db()->prepare(
        "SELECT DISTINCT p.id, p.year, p.month
         FROM sc_periods p INNER JOIN $table t ON t.period_id = p.id
    ...
    foreach ($periods as $per) {
        $pid       = (int)$per['id'];
        $year      = (int)$per['year'];
        $monthName = $MONTHS[(int)$per['month'] - 1] ?? 'Unknown';
        $data = match($module) {
            'finance'     => loadFinance($pid),
            'planning'    => loadPlanning($pid),
            'procurement' => loadProcurement($pid),
...
function handleSave(): never {
    ...
    // Write to sync log so other connected devices know to reload
    try {
        db()->prepare(
            'INSERT INTO sc_sync_log (module, period_id, saved_by, saved_at)
             VALUES (?, ?, ?, NOW())
             ON DUPLICATE KEY UPDATE saved_by=VALUES(saved_by), saved_at=NOW()'
        )->execute([$module, $pid, (int)$sess['user_id']]);
    } catch (Throwable) {}
