...
const KPI_REGISTRY = [
    ...
    // ── Finance ──────────────────────────────────────────────
    {
        ...
        resolver: (y, m) => {
            ...
            return { value: pct, raw: a, formatted: pct !== null ? pct.toFixed(2) + '%' : '—', sub: fmtN(a, 2) + ' MT', isPercent: true };
        }
    },
    {
        ...
        resolver: (y, m) => {
            ...
            return { value: pct, raw: a, formatted: pct !== null ? pct.toFixed(2) + '%' : '—', sub: fmtC(a), isPercent: true };
        }
    },
    {
    ...
    },
    {
        ...
        resolver: (y, m) => {
            ...
            return { value: pct, raw: tot, formatted: pct.toFixed(2) + '%', sub: fmtN(tot) + ' OT hrs', isPercent: true };
        }
    },
    ...
    // ── SC Planning ──────────────────────────────────────────
    {
        ...
        resolver: (y, m) => {
            ...
            return { value: avg, raw: avg, formatted: avg.toFixed(2) + '%', sub: vals.length + ' regions', isPercent: true };
        }
    },
    {
        ...
        resolver: (y, m) => {
            ...
            if (!vals.length) return null;
            const avg = vals.reduce((a, b) => a + b, 0) / vals.length;
            return { value: avg, raw: avg, formatted: avg.toFixed(2) + '%', sub: 'Indonesia, India, Direct', isPercent: true };
        }
    },
    {
    ...
    },
    {
        ...
        resolver: (y, m) => {
            ...
            return { value: acc, raw: acc, formatted: acc.toFixed(2) + '%', sub: fmtN(cnt, 0) + ' counted', isPercent: true };
        }
    },
    ...
    // ── Procurement ──────────────────────────────────────────
    {
        ...
        resolver: (y, m) => {
            ...
            if (!t) return null;
            const pct = a / t * 100;
            return { value: pct, raw: a, formatted: pct.toFixed(2) + '%', sub: fmtC(a) + ' saved', isPercent: true };
        }
    },
    ...
    // ── Production — Utilization ─────────────────────────────
    {
        ...
        resolver: (y, m) => {
            ...
            if (!tt) return null;
            const pct = tp / tt * 100;
            return { value: pct, raw: pct, formatted: pct.toFixed(2) + '%', sub: fmtN(tp) + ' productive hrs', isPercent: true };
        }
    },
    {
    ...
    },
    {
        ...
        resolver: (y, m) => {
            ...
            if (!tp) return null;
            const pct = ta / tp * 100;
            return { value: pct, raw: ta, formatted: pct.toFixed(2) + '%', sub: fmtN(ta) + ' kg actual', isPercent: true };
        }
    },
    {
    ...
    },
    {
        ...
        resolver: (y, m) => {
            ...
            return { value: pct, raw: otH, formatted: pct.toFixed(2) + '%', sub: fmtN(otH) + ' OT hrs', isPercent: true };
        }
    },
    {
...
function anResolveYearValue(kpi, year) {
    ...
    const results = MONTHS.map(m => _anResolve(kpi, year, m)).filter(Boolean);
    if (!results.length) return null;
    const avg = results.reduce((s, r) => s + (r.value || 0), 0) / results.length;
    return { value: avg, raw: avg, formatted: avg.toFixed(2) + (kpi.unit === '%' ? '%' : ''), sub: results.length + ' months avg', isPercent: kpi.unit === '%' };
}
...
// ── Render ────────────────────────────────────────────────────
// ── Resolver cache — keyed by kpi.id + year + month ──────────
// Cleared whenever period filter changes or data is saved.
const _anCache = new Map();
function _anCacheKey(kpiId, year, month) { return `${kpiId}|${year}|${month}`; }
function anClearCache() { _anCache.clear(); }

function _anResolve(kpi, year, month) {
    const k = _anCacheKey(kpi.id, year, month);
    if (_anCache.has(k)) return _anCache.get(k);
    const v = kpi.resolver(year, month);
    _anCache.set(k, v);
    return v;
}

let _anRenderToken = 0; // cancel stale renders
...
function anRender() {
    ...
    // Show skeleton placeholders immediately so the page feels instant
    container.innerHTML = kpis.map(k =>
        `<div class="a-card a-card-skeleton" id="an-sk-${k.id}">
    ...
    // Render cards in small async chunks so the main thread stays responsive
    const token = ++_anRenderToken;
    const CHUNK = 4; // cards per frame
    let   idx   = 0;

    // Pre-compute all card data synchronously but outside the DOM
    const cardData = kpis.map(kpi => {
        const vals   = periods.map(p =>
            (anState.rangeMode === 'year' && !p.month)
                ? anResolveYearValue(kpi, p.year)
                : _anResolve(kpi, p.year, p.month)
        );
    ...
    // Update summary immediately from pre-computed data
    const withData = cardData.filter(d => d.latest != null).length;
    ...
    function renderChunk() {
        if (token !== _anRenderToken) return; // stale — newer render started
        const end = Math.min(idx + CHUNK, cardData.length);
        for (let i = idx; i < end; i++) {
            const { kpi, vals, latest, trend } = cardData[i];
            const el  = document.getElementById(`an-sk-${kpi.id}`);
            if (!el) continue;
            // Replace skeleton in-place to avoid layout thrash
            const div = document.createElement('div');
            div.innerHTML = anCardHTML(kpi, vals, latest, trend, periods);
            const card = div.firstElementChild;
            if (card) el.replaceWith(card);
        }
        idx = end;
        if (idx < cardData.length) requestAnimationFrame(renderChunk);
    }
...
function anCardHTML(kpi, vals, latest, trend, periods) {
    ...
    if (trend.pct !== null) {
        ...
        trendHTML = `<span class="a-card-trend ${trend.dir}">${trend.dir === 'up' ? '▲' : trend.dir === 'down' ? '▼' : '→'} ${sign}${trend.pct.toFixed(2)}%</span>`;
    }
...
function anApplyRange() {
    anReadRangeControls();
    anClearCache(); // period changed — invalidate resolver cache
    anRender();
}
...
/* ── Inline skeleton styles for analytics cards (added here to avoid a new CSS file) ── */
(function() {
    ...
    style.textContent = `
        ...
        .sk-line { background: linear-gradient(90deg,var(--border-light) 25%,#f0f0f0 50%,var(--border-light) 75%); background-size:200% 100%; animation:skShimmer 1.2s infinite; border-radius:4px; }
        .sk-title { height:14px; width:60%; margin-bottom:12px; }
        .sk-val   { height:22px; width:40%; margin-top:10px; }
        .sk-chart { height:56px; background:linear-gradient(90deg,var(--border-light) 25%,#f0f0f0 50%,var(--border-light) 75%); background-size:200% 100%; animation:skShimmer 1.2s infinite; border-radius:4px; }
        @keyframes skShimmer { 0%{background-position:200% 0} 100%{background-position:-200% 0} }
    `;
    if (!document.getElementById('an-skeleton-styles')) {
        style.id = 'an-skeleton-styles';
