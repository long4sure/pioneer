...
const moduleSubs = {
    ...
    sysadmin:   'System Admin Panel — Data, Users & Audit Log',
};
...
async function switchModule(mod, el) {
    // Show loading overlay for slower modules that hit the DB
    const needsLoad = (mod !== 'home');
    if (needsLoad) _showModuleLoader(moduleNames[mod] || mod);
    ...
    // Load template dynamically if empty
    const pg = document.getElementById('mod-' + mod);
    if (pg && pg.innerHTML.trim() === '') {
        try {
            // cache busting optional, but good for dev
            const resp = await fetch('templates/' + mod + '.html?v=' + Date.now());
            if (resp.ok) pg.innerHTML = await resp.text();
        } catch (e) {
            console.error('Failed to load template:', mod, e);
        }
    }
    ...
    } else if (mod === 'sysadmin') {
        setTimeout(() => { saInit(); _hideModuleLoader(); }, 60);
    } else {
        // For data modules: load from DB then hide loader
        const done = () => _hideModuleLoader();
        if (typeof DB_MODE !== 'undefined' && DB_MODE === 'online') {
            globalFilterChange();
            setTimeout(done, 400); // give DB load time
        } else {
            globalFilterChange();
            setTimeout(done, 80);
        }
...
function _showModuleLoader(name) {
    clearTimeout(_loaderTimer);
    let el = document.getElementById('module-loader');
    if (!el) return;
    el.querySelector('.ml-name').textContent = name;
    el.classList.add('visible');
    // Safety fallback — always hide after 3s even if load hangs
    _loaderTimer = setTimeout(_hideModuleLoader, 3000);
}
function _hideModuleLoader() {
    clearTimeout(_loaderTimer);
    const el = document.getElementById('module-loader');
