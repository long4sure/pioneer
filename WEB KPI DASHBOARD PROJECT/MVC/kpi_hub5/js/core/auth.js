/* ============================================================
   ...
   Roles: systemadmin | User | viewer
   ============================================================ */

// ── Built-in accounts ─────────────────────────────────────────
const BUILTIN_USERS = {
    'systemadmin': { role:'systemadmin',   password:'Pioneer@Sysad2026', displayName:'System Admin',   builtin:true,  status:'active', createdAt:'2026-01-01' },
    'user':      { role:'user',      password:'Pioneer@User2026', displayName:'User', builtin:true,  status:'active', createdAt:'2026-01-01' },
    'viewer':     { role:'viewer',     password:'Pioneer@View2026',  displayName:'Viewer',        builtin:false, status:'active', createdAt:'2026-01-01' },
};
...
function authLogin() {
    ...
    setTimeout(()=>{
        ...
        if (user.status==='pending') { _loginErr('Your account is pending approval by an Useristrator.'); if(bEl){bEl.disabled=false;bEl.textContent='Sign In';} return; }
        if (user.status==='rejected') { _loginErr('Your registration was not approved. Contact an Useristrator.'); if(bEl){bEl.disabled=false;bEl.textContent='Sign In';} return; }
...
function authLogout() {
    ...
    document.body.classList.remove('role-User','role-viewer','role-systemadmin');
    _showLogin();
}
...
function authRegister() {
    ...
    document.getElementById('reg-panel').innerHTML = `
      <div style="text-align:center;padding:24px 0 8px">
        ...
        <div class="login-subheading">Your request for <strong>@${_esc(uname)}</strong> has been sent for approval. You'll be notified once an Useristrator reviews it.</div>
...
function openUserMgmt() {
    if (authSession.role!=='systemadmin') return;
...
function renderUserMgmt() {
    const users   = usersGet();
    const pending = pendingGet();
    const rb = r => ({systemadmin:'role-systemadmin',User:'role-User',viewer:'role-viewer'}[r]||'');
    ...
    const rows = Object.entries(users).filter(([,u])=>u.status==='active'||!u.status)
      .map(([uname,u])=>`
        <div class="um-row">
          ...
          <div class="um-row-actions">
            ${uname!=='systemadmin'?`<select class="um-role-select" onchange="changeUserRole('${uname}',this.value)">
...
function changeUserRole(username, newRole) {
    if (username==='systemadmin') { showToast('Cannot change systemadmin role.'); renderUserMgmt(); return; }
    const users = usersGet();
...
function _applyRole(role) {
    ...
    if (umNav) umNav.style.display = (role==='systemadmin')?'':'none';
}

// ── Topbar ────────────────────────────────────────────────────
function _updateTopbar() {
    const ICONS = {
        systemadmin:`<svg viewBox="0 0 16 16" fill="currentColor" width="10"><path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/></svg>`,
        User:`<svg viewBox="0 0 16 16" fill="currentColor" width="10"><path d="M8 1a2.5 2.5 0 011.5 4.5L10 8l3 2-1 1.5-3-2v3H7v-3L4 11.5 3 10l3-2-.5-2.5A2.5 2.5 0 018 1z"/></svg>`,
        viewer:`<svg viewBox="0 0 16 16" fill="currentColor" width="10"><path d="M8 8a3 3 0 100-6 3 3 0 000 6zm-5 6a5 5 0 0110 0H3z"/></svg>`,
    };
    const LABELS = { systemadmin:'Super User', User:'User', viewer:'Viewer' };
    ...
    if (authSession.role==='systemadmin') {
...
function _regPanelHTML() {
    return `<div class="login-heading">Request Access</div>
      <div class="login-subheading">Your account will be active after Useristrator approval.</div>
      ...
      <div class="login-field"><label class="login-label">Requested Role</label>
        <select class="login-input" id="reg-role" style="cursor:pointer">
          ...
          <option value="User">User — full data-entry access</option>
        </select>
      </div>
