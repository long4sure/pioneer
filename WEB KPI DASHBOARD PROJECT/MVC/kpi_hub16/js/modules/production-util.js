...
async function utilClearLine() {
    if (!await appConfirm('Clear Line Data', 'Clear all utilization hours for this line?', 'Clear', false)) return;
    const l = document.getElementById('util-lineSelect').value;
    if (!l) return;
    const d = utilGetDB()[l];
...
async function utilClearAll() {
    if (!await appConfirm('Clear ALL Utilization Data', 'This will erase all Production Utilization data. This cannot be undone.', 'Clear All', true)) return;
