...
const KPI_REGISTRY = [
    ...
    },
    {
        ...
        resolver: (y, m) => {
            ...
            const pct = th > 0 ? tot / th * 100 : null;
            return { value: pct, raw: tot, formatted: pct !== null ? pct.toFixed(1) + '%' : '—', sub: fmtN(tot) + ' OT hrs', isPercent: true };
        }
    },
    ...
    },
    {
        ...
        thresholds: { good: 98, warn: 85, higherIsBetter: true },
        resolver: (y, m) => {
            const d = whDB[y]?.[m]; if (!d) return null;
...
function anBarColor(kpi, val) {
    ...
    if (!thresholds.good) return kpi.color;
    const { cls } = statusBadge(val.value, thresholds);
...
function anRender() {
    ...
    // Summary counts
    const withData = cardData.filter(d => d.latest).length;
    const goodCnt  = cardData.filter(d => d.latest && d.kpi.thresholds.good && statusBadge(d.latest.value, d.kpi.thresholds).cls !== 'badge-critical').length;
    const badCnt   = cardData.filter(d => d.latest && d.kpi.thresholds.good && statusBadge(d.latest.value, d.kpi.thresholds).cls === 'badge-critical').length;
...
function anCardHTML(kpi, vals, latest, trend, periods) {
    ...
    const periodsRowsHTML = periods.slice(-8).map((p, i) => {
        ...
        let bc = '', bl = '';
        if (kpi.thresholds.good) { const s = statusBadge(v.value, kpi.thresholds); bc = s.cls; bl = s.label; }
