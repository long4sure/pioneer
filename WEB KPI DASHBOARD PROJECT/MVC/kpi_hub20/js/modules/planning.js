...
async function plClearCat() {
    if (!await appConfirm('Clear Current Category', 'Clear the current input fields for this category?', 'Clear', false)) return;
    const cat = document.getElementById('pl-inputCat')?.value;
    if (!cat || !plCatDefs[cat]) return;
    plCatDefs[cat].fields.forEach(([, key]) => {
        const visible = document.getElementById('plc-' + key);
...
async function plClearAll() {
    if (!await appConfirm('Clear ALL Planning Data', 'This will erase SC Planning data for every period. This cannot be undone.', 'Clear All', true)) return;
