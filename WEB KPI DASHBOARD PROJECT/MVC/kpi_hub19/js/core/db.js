...
async function dbLoadPeriod() {
    ...
    // ── Finance ──────────────────────────────────────────────
    // Check that at least one value is non-empty (not just the empty scaffold)
    if (all.finance?.prod && (all.finance.prod.actual || all.finance.prod.target ||
        all.finance.op?.actual || all.finance.cap?.actual)) {
        const d = all.finance;
        finDB[year][month] = {
    ...
    // ── Procurement ───────────────────────────────────────────
    if (all.procurement && (all.procurement.actual || all.procurement.target)) {
