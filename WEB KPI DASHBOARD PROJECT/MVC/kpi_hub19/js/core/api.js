...
const API = (() => {
    ...
    async function loadAll(year, month) {
        ...
        // PHP ok() wraps as { ok, msg, data: { year, month, data: { finance, planning, ... } } }
        // _fetch returns json.data which is { year, month, data: { ... } }
        // We need the inner data object containing the actual module data
        if (res.data && typeof res.data === 'object') return res.data;
        return res;
    }

    // Load ALL saved periods for one module (for sysadmin Data Records)
    ...
    async function load(module, year, month) {
        ...
        if (!res || res.error) return null;
        // Same unwrapping: res is { year, month, data: { actual module data } }
        if (res.data && typeof res.data === 'object') return res.data;
        return res;
    }
