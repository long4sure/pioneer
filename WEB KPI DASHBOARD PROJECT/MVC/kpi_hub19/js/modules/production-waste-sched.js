...
async function wasteClearLine() {
    if (!await appConfirm('Clear Line Data', 'Clear output/waste data for this line?', 'Clear', false)) return;
    const l = document.getElementById('waste-lineSelect').value; if (!l) return;
    const d = wasteGetDB()[l];
...
async function wasteClearAll() {
    if (!await appConfirm('Clear ALL Waste Data', 'This will erase all Output & Waste data. This cannot be undone.', 'Clear All', true)) return;
...
async function schedClearLine() {
    if (!await appConfirm('Clear Line Data', 'Clear scheduling data for this line?', 'Clear', false)) return;
    const l = document.getElementById('sched-lineSelect').value; if (!l) return;
    const d = schedGetDB()[l]; d.actual = ''; d.planned = '';
    schedLoadLine(); schedCalcAll();
}
async function schedClearAll() {
    if (!await appConfirm('Clear ALL Scheduling Data', 'This will erase all Scheduling data. This cannot be undone.', 'Clear All', true)) return;
