...
function handleLoad(): never {
    ...
    // Special case: year=all → return all periods for this module
    if ($yearRaw === 'all') {
        loadAllPeriods($module);
        return;
    }
...
// Load ALL saved periods for one module (used by sysadmin Data Records)
function loadAllPeriods(string $module): never {
    $MONTH_NAMES = ['January','February','March','April','May','June',
                    'July','August','September','October','November','December'];

    // Get all period IDs that have data for this module
    $tableMap = [
        'finance'     => 'sc_finance',
        'planning'    => 'sc_planning',
        'procurement' => 'sc_procurement',
        'prod_util'   => 'sc_prod_util',
        'prod_waste'  => 'sc_prod_waste',
        'prod_sched'  => 'sc_prod_sched',
        'warehouse'   => 'sc_warehouse',
    ];
    $table = $tableMap[$module] ?? null;
    if (!$table) fail("Unknown module: $module");

    // Join with periods to get year/month for each row
    $stmt = db()->query(
        "SELECT DISTINCT p.id, p.year, p.month
         FROM sc_periods p
         INNER JOIN $table t ON t.period_id = p.id
    ...
    foreach ($periods as $per) {
        $pid  = (int)$per['id'];
        $year = (int)$per['year'];
        $mon  = (int)$per['month'] - 1; // 0-indexed
        $monthName = $MONTH_NAMES[$mon] ?? 'Unknown';
