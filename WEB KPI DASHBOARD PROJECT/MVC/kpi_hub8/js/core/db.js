...
async function dbBoot() {
    ...
    if (DB_MODE === 'online') {
        console.log('[DB] MySQL connected:', API.base);
        await dbLoadPeriod();
        _dbPatchAuth();
        _dbPatchSaves();
        _dbPatchFilter();
        ...
        console.log('[DB] Offline — using localStorage');
    }
}
...
function _dbBadge(mode) {
    ...
    if (mode === 'online') {
        ...
        el.innerHTML = `<svg viewBox="0 0 16 16" fill="currentColor" width="9"><path d="M8 0C5.24 0 3 .9 3 2v12c0 1.1 2.24 2 5 2s5-.9 5-2V2c0-1.1-2.24-2-5-2zm3 14c0 .55-1.34 1-3 1s-3-.45-3-1v-2.09c.85.38 1.9.59 3 .59s2.15-.21 3-.59V14zm0-4c0 .55-1.34 1-3 1s-3-.45-3-1V7.91c.85.38 1.9.59 3 .59s2.15-.21 3-.59V10zm0-4c0 .55-1.34 1-3 1s-3-.45-3-1V3.91c.85.38 1.9.59 3 .59s2.15-.21 3-.59V6z"/></svg> MySQL`;
    } else {
        el.style.background = 'rgba(200,32,42,0.08)';
        el.style.color      = 'var(--pioneer-red)';
        el.title            = 'Offline — data stored locally';
        el.innerHTML = `<svg viewBox="0 0 16 16" fill="currentColor" width="9"><path d="M8 0C5.24 0 3 .9 3 2v12c0 1.1 2.24 2 5 2s5-.9 5-2V2c0-1.1-2.24-2-5-2zm3 14c0 .55-1.34 1-3 1s-3-.45-3-1v-2.09c.85.38 1.9.59 3 .59s2.15-.21 3-.59V14zm0-4c0 .55-1.34 1-3 1s-3-.45-3-1V7.91c.85.38 1.9.59 3 .59s2.15-.21 3-.59V10zm0-4c0 .55-1.34 1-3 1s-3-.45-3-1V3.91c.85.38 1.9.59 3 .59s2.15-.21 3-.59V6z"/></svg> Offline`;
    }
}
