...
const API = (() => {
    ...
    const BASE = (() => {
        ...
        if (protocol === 'file:') return null; // offline
        if (hostname === 'localhost' || hostname === '127.0.0.1') {
            // e.g. /kpi_hub/index.html  →  /kpi_hub/api
            const folder = pathname.split('/').slice(0, -1).join('/');
            return `${protocol}//${hostname}${port ? ':'+port : ''}${folder}/api`;
        }
        return null; // unknown host — offline fallback
    })();
    ...
    async function checkOnline() {
        ...
        try {
            const r = await fetch(`${BASE}/auth.php?action=me`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' },
                signal: AbortSignal.timeout(2500),
            });
            _online = (r.status !== 0); // any HTTP response means server is up
        } catch(e) {
            _online = false;
        }
        return _online;
