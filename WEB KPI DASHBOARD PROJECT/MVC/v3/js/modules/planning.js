...
function plRender() {
    ...
    // FG inventory KGS
    const fkFG=gv('fgKgsFG'),fkFN=gv('fgKgsFN'),fkSL=gv('fgKgsSL'),fkNM=gv('fgKgsNM'),fkEX=gv('fgKgsEX'),fkDays=g('fgKgsDays');
    const fkTot=fkFG+fkFN+fkSL+fkNM+fkEX;
    const hasKgs = [g('fgKgsFG'),g('fgKgsFN'),g('fgKgsSL'),g('fgKgsNM'),g('fgKgsEX')].some(v=>v!==null);
    setText('pl-fgKgsFG',fmtN(fkFG));setText('pl-fgKgsFN',fmtN(fkFN));setText('pl-fgKgsSL',fmtN(fkSL));setText('pl-fgKgsNM',fmtN(fkNM));setText('pl-fgKgsEX',fmtN(fkEX));
    ...
    // FG inventory PHP
    const fpFG=gv('fgPhpFG'),fpFN=gv('fgPhpFN'),fpSL=gv('fgPhpSL'),fpNM=gv('fgPhpNM'),fpEX=gv('fgPhpEX');
    const fpTot=fpFG+fpFN+fpSL+fpNM+fpEX;
    const hasPhp = [g('fgPhpFG'),g('fgPhpFN'),g('fgPhpSL'),g('fgPhpNM'),g('fgPhpEX')].some(v=>v!==null);
    setText('pl-fgPhpFG',fmtC(fpFG));setText('pl-fgPhpFN',fmtC(fpFN));setText('pl-fgPhpSL',fmtC(fpSL));setText('pl-fgPhpNM',fmtC(fpNM));setText('pl-fgPhpEX',fmtC(fpEX));
    ...
    // RM/PM KGS
    const rkFG=gv('rmKgsFG'),rkFN=gv('rmKgsFN'),rkSL=gv('rmKgsSL'),rkNM=gv('rmKgsNM'),rkEX=gv('rmKgsEX'),rkDays=g('rmKgsDays');
    const rkTot=rkFG+rkFN+rkSL+rkNM+rkEX;
    const hasRkgs = [g('rmKgsFG'),g('rmKgsFN'),g('rmKgsSL'),g('rmKgsNM'),g('rmKgsEX')].some(v=>v!==null);
    setText('pl-rmKgsFG',fmtN(rkFG));setText('pl-rmKgsFN',fmtN(rkFN));setText('pl-rmKgsSL',fmtN(rkSL));setText('pl-rmKgsNM',fmtN(rkNM));setText('pl-rmKgsEX',fmtN(rkEX));
    ...
    // RM/PM PHP
    const rpFG=gv('rmPhpFG'),rpFN=gv('rmPhpFN'),rpSL=gv('rmPhpSL'),rpNM=gv('rmPhpNM'),rpEX=gv('rmPhpEX');
    const rpTot=rpFG+rpFN+rpSL+rpNM+rpEX;
    const hasRphp = [g('rmPhpFG'),g('rmPhpFN'),g('rmPhpSL'),g('rmPhpNM'),g('rmPhpEX')].some(v=>v!==null);
    setText('pl-rmPhpFG',fmtC(rpFG));setText('pl-rmPhpFN',fmtC(rpFN));setText('pl-rmPhpSL',fmtC(rpSL));setText('pl-rmPhpNM',fmtC(rpNM));setText('pl-rmPhpEX',fmtC(rpEX));
    ...
    setText('pl-fgAccDet', fgAcc !== null ? fgAcc.toFixed(1) + '%' : '—');
    setText('pl-fgAcc',    fgAcc !== null ? fgAcc.toFixed(1) + '%' : '—');
    setText('pl-rmCnt',   rmCnt  !== null ? fmtN(rmCnt)  : '—');
    setText('pl-rmMiss',  rmMiss !== null ? fmtN(rmMiss) : '—');
    setText('pl-rmAccDet', rmAcc !== null ? rmAcc.toFixed(1) + '%' : '—');
    setText('pl-rmAcc',    rmAcc !== null ? rmAcc.toFixed(1) + '%' : '—');

    // Forecast — local
    const localIds = ['fcGma','fcNorth','fcSouth','fcVis','fcMind','fcMT','fcPsbsi'];
    const localVals = localIds.map(id => g(id));
    localIds.forEach((id, i) => setText('pl-' + id, localVals[i] !== null ? localVals[i].toFixed(1) + '%' : '—'));
    ...
    expIds.forEach((id, i) => setText('pl-' + id, expVals[i] !== null ? expVals[i].toFixed(1) + '%' : '—'));
    ...
    prodIds.forEach((id, i) => setText('pl-' + id, prodVals[i] !== null ? prodVals[i].toFixed(1) + '%' : '—'));
    ...
    setText('pl-fcOverall', allFilled.length ? (allFilled.reduce((a,b)=>a+b,0) / allFilled.length).toFixed(1) + '%' : '—');
}
