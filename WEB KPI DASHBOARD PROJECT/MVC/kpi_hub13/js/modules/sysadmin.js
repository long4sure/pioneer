...
async function saDeleteUser(username) {
    if (!await confirmDialog({title:'Delete User',message:`Permanently delete @${username}? This cannot be undone.`,confirmLabel:'Delete',danger:true})) return;
...
async function saRejectPendingByName(username) {
    if (!await confirmDialog({title:'Reject Registration',message:`Reject the registration request from @${username}? They will need to register again.`,confirmLabel:'Reject',danger:true})) return;
...
function saRenderData() {
    ...
    setHTML('sa-data-thead', `<tr>${headers}<th style="width:130px">Actions</th></tr>`);

    const tbody = page.map(row => {
        // row[0]=year, row[1]=month, row[2+]=data, row may have row[2]=line for production
        const year  = row[0]; const month = row[1];
        const line  = (module.startsWith('production_') || module === 'production_util' || module === 'production_waste' || module === 'production_sched') ? row[2] : null;
        const editLabel = line ? `${year}/${month}/${line}` : `${year}/${month}`;
        const cells = row.map((v, i) => `<td class="${i < 2 ? 'mono nowrap' : 'dim'}" style="max-width:160px;overflow:hidden;text-overflow:ellipsis">${_esc(String(v ?? '—'))}</td>`).join('');
        return `<tr>
            ${cells}
            <td class="nowrap" style="width:130px">
                <button class="sa-action-btn sa-action-edit" style="font-size:10px" onclick="saDataGoEdit('${_esc(module)}','${_esc(String(year))}','${_esc(String(month))}','${line ? _esc(String(line)) : ''}')">✏ Edit</button>
                <button class="sa-action-btn sa-action-delete" style="font-size:10px;margin-left:4px" onclick="saDataDelete('${_esc(module)}','${_esc(String(year))}','${_esc(String(month))}','${line ? _esc(String(line)) : ''}')">🗑</button>
            </td>
        </tr>`;
    }).join('');
...
// Navigate to the module's data entry tab for editing
function saDataGoEdit(module, year, month, line) {
    // Map module name to nav module + submod
    const MOD_MAP = {
        finance: 'finance', planning: 'planning', procurement: 'procurement',
        production_util: 'production', production_waste: 'production',
        production_sched: 'production', warehouse: 'warehouse',
    };
    const SUBMOD_MAP = {
        production_util: 'util', production_waste: 'waste', production_sched: 'sched',
    };
    const navMod  = MOD_MAP[module] || module;
    const subMod  = SUBMOD_MAP[module];

    // Set the global period selectors to the target year/month
    const yrEl = document.getElementById('globalYear');
    const moEl = document.getElementById('globalMonth');
    if (yrEl) { for (let o of yrEl.options) o.selected = (o.value === year || o.text === year); }
    ...
    setTimeout(() => {
        ...
        const inputBtn = document.querySelector(`#mod-${navMod} .tab-btn[data-tab="input"]`);
        if (inputBtn) inputBtn.click();
        // If production with a line, select it
        if (line) {
            const lineEl = document.getElementById(`${subMod || module}-lineSelect`);
            if (lineEl) { lineEl.value = line; lineEl.dispatchEvent(new Event('change')); }
        }
        showToast(`Editing ${module} — ${month} ${year}${line ? ' / ' + line : ''}`);
    }, 300);
}

// Delete a period's data record
async function saDataDelete(module, year, month, line) {
    const label = `${module} data for ${month} ${year}${line ? ' / ' + line : ''}`;
    const ok = await confirmDialog({
        title: 'Delete Record',
        message: `Delete ${label}? The data will be cleared from memory. This cannot be undone.`,
    ...
    // Clear the in-memory DB for this record
    if (module === 'finance' && finDB[year]?.[month]) {
        finDB[year][month] = { prod:{actual:'',target:''}, op:{actual:'',budget:''}, cap:{actual:'',budget:''}, other:{actual:'',budget:''}, labor:{dl:{reg:'',ot:''},il:{reg:'',ot:''}} };
    } else if (module === 'procurement' && procDB[year]?.[month]) {
        procDB[year][month] = { actual:'', target:'' };
    } else if (module === 'planning' && plDB[year]?.[month]) {
        const keys = Object.keys(plDB[year][month]);
        keys.forEach(k => { plDB[year][month][k] = ''; });
    } else if (module === 'warehouse' && whDB[year]?.[month]) {
        // Reset warehouse for this period
        const m = MONTHS_ARR.indexOf(month) + 1;
        if (m > 0) {
            // Re-init this period
            const init = { otif:{served:'',total:''}, vol:{del:'',ord:''}, fr:{sc:{del:'',ord:''},corp:{del:'',ord:''},core:{del:'',ord:''},m7:{del:'',ord:''}}, wh:{rmTot:'',rmUsed:'',fgTot:'',fgUsed:'',extTot:'',extUsed:''}, otdl:{gma:'',north:'',central:'',south:'',vis:'',mind:'',mt:'',pai:''}, trucks:{t10:'',auv:'',t6:'',t4:'',c20:''}, mp:{reg:'',agy:'',res:'',regH:'',agyH:'',otH:'',abs:'',days:''}, nonOtif:{}, soVal:{}, soWt:{} };
            whDB[year][month] = init;
        }
    } else if ((module === 'production_util') && utilDB[year]?.[month]?.[line]) {
        const keys = Object.keys(utilDB[year][month][line]);
        keys.forEach(k => { utilDB[year][month][line][k] = ''; });
    } else if ((module === 'production_waste') && wasteDB[year]?.[month]?.[line]) {
        wasteDB[year][month][line] = { fg:'', rep:'', rej:'', waste:'' };
    } else if ((module === 'production_sched') && schedDB[year]?.[month]?.[line]) {
        schedDB[year][month][line] = { actual:'', planned:'' };
    }

    // If online, sync the cleared data to MySQL
    if (typeof DB_MODE !== 'undefined' && DB_MODE === 'online') {
        const m = MONTHS_ARR.indexOf(month) + 1;
        if (m > 0) {
            const payload = line ? { line } : {};
            dbSync?.(module, payload);
        }
    }

    saLog('delete', 'Data', `Deleted ${label}`);
...
function saClearLog() {
    if (!await confirmDialog({title:'Clear Audit Log',message:'Delete all audit log entries? This cannot be undone.',confirmLabel:'Clear Log',danger:true})) return;
    saLogSave([]);
    saRenderLog();
