...
async function procClear() {
    if (!await confirmDialog({title:"Clear Data",message:"Clear current procurement input values?",confirmLabel:"Clear",danger:false})) return;
    const ea = document.getElementById('proc-i-actual');
    const et = document.getElementById('proc-i-target');
    if (ea) ea.value = '';
    if (et) et.value = '';
...
async function procClearAll() {
    if (!await confirmDialog({title:"Clear All Data",message:"Clear ALL Procurement data? This cannot be undone.",confirmLabel:"Clear All",danger:true})) return;
