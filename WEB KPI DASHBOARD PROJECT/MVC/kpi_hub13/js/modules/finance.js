...
async function finClear() {
    if (!await confirmDialog({title:"Clear Data",message:"Clear the current input values? Saved data is not affected until you save.",confirmLabel:"Clear",danger:false})) return;
    document.querySelectorAll('#fin-inputSections input').forEach(e => e.value = '');
    finSave();
}
async function finClearAll() {
    if (!await confirmDialog({title:"Clear All Data",message:"Clear ALL Finance data for ALL periods? This cannot be undone.",confirmLabel:"Clear All",danger:true})) return;
