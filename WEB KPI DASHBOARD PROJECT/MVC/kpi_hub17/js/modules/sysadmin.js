...
function saRenderData() {
    ...
    // Filter by year and month first (row[0]=year, row[1]=month)
    let filtered = rows;
...
function saDataExport(format) {
    ...
    if (format === 'excel') {
        // Build an HTML table — Excel opens it natively
        const esc = v => String(v ?? '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
        const thead = schema.map(c => `<th>${esc(c)}</th>`).join('');
        const tbody = rows.map(r => `<tr>${r.map(v=>`<td>${esc(String(v??''))}</td>`).join('')}</tr>`).join('');
        const xls   = `<html xmlns:o="urn:schemas-microsoft-com:office:office"
xmlns:x="urn:schemas-microsoft-com:office:excel"
xmlns="http://www.w3.org/TR/REC-html40">
<head><meta charset="UTF-8">
<!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets>
<x:ExcelWorksheet><x:Name>${_esc(module)}</x:Name>
<x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions>
        ...
        const a    = Object.assign(document.createElement('a'), {
            href:     URL.createObjectURL(blob),
            download: `${module}_${mo}_${yr}.xls`
        });
        document.body.appendChild(a); a.click();
        setTimeout(() => { URL.revokeObjectURL(a.href); a.remove(); }, 200);
        showToast(`Exported ${rows.length} records as Excel`);
        saLog('export', 'Data Records', `${module} ${mo} ${yr} Excel`);
    }
}
